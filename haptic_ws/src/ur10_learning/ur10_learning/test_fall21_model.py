...
class UR10(Node):
    def __init__(self, urdf_path, home_state, folderpath):
        super().__init__('test_fall21_model')
    ...
    def get_uncertainty_end_eff(self):
        ...
        uncertainty = np.mean(action_std)
        uncertainty = uncertainty*2 #Scale uncertainty
        uncertainty = 1 / (1 + np.exp(-25*(uncertainty-0.28))) # Amplify Uncertainty: Make small values smaller and big values bigger
...
def main(args=None):
    ...
    # Take user input asking which segment to exclude
    exclude_segment = input("[*] Enter the segment you want to exclude (hint: 'segment1', 'segment2', 'segment3'): ")
    if exclude_segment == 'segment1':
        segment_name = 'segment23'
    elif exclude_segment == 'segment2':
        segment_name = 'segment13'
    elif exclude_segment == 'segment3':
        segment_name = 'segment12'
    else:
        segment_name = 'segment123'
    
    folderpath = '~/ros_projects/haptic_ws/src/ur10_learning/data/demonstrations/segments/models/' + segment_name + '/'
