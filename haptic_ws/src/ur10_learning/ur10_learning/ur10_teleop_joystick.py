...
class UR10Teleop(Node):
    ...
    def handle_joystick_input(self):
        ...
        elif self.record and self.curr_time - self.last_time > time_step:
...
def main(args=None):
    ...
    filename = '~/ros_projects/haptic_ws/src/ur10_learning/data/recording.pkl'
    filename = os.path.expanduser(filename)
    print("Filename is: ", filename)
    teleop_node = UR10Teleop(urdf_path, filename)
    home = np.array([-1.77641041, -2.33904296, -2.1207298,  -0.26323218,  1.59763741,  2.60214544])
    # ptA = np.array([-1.69330895, -2.98944648, -0.48538898, -1.25135056,  1.59884996,  2.68512211])
    # ptB = np.array([-1.21463909, -3.08015368, -0.28245062, -1.34890968,  1.60242182,  3.16400266])
    # ptC = np.array([-0.98621028, -2.39054328, -1.89082655, -0.42004879,  1.59802104,  3.39262436])
