...
class UR10(Node):
    def __init__(self, urdf_path, home_state, folderpath):
        super().__init__('test_segment_model')
    ...
    def handle_joystick_input(self):
        ...
        if self.test:
            uncertainty = self.get_uncertainty_end_eff()
...
def main(args=None):
    ...
    segment_name = input("[*] Enter the segment you want to test (hint: 'segment1', 'segment2', 'segment3'): ")
    if segment_name == '':
